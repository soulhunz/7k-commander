...
เอกสารนี้คู่กับการแก้ฝั่งหน้าเว็บใน `index.html` และฝั่งเซิร์ฟเวอร์ใน `code.gs`

> **อัปเดต 2026-09-15** — ข้อ 1 และ 2 ปิดที่ฝั่งเซิร์ฟเวอร์แล้ว ต้อง Deploy `code.gs` เวอร์ชันใหม่จึงจะมีผล
...
| # | ปัญหา | สถานะ |
|---|---|---|
| 1 | พอร์ทัลลูกกิลโหลดข้อมูล **ทุกคนทุกกิลด์** รวมรหัสทุกคน | ✅ แก้แล้ว — `getAllData`/`getUpdates` ล้างช่อง `accessCode` ทิ้งให้ทุกคนที่ไม่ใช่แอดมิน (`stripAccessCodes_`) |
| 2 | ตรวจรหัสฝั่งหน้าเว็บล้วน ปลอมตัวเป็นคนอื่นได้ | ✅ แก้แล้ว — เพิ่ม action `verifyMemberCode`, และ `getMemberInventory` / `saveMemberInventory` บังคับให้ส่งรหัสที่ตรงกับแถวจริง |
...
| 5 | รหัสสั้นอย่าง `001` ผ่านได้ | ✅ บังคับอย่างน้อย 6 ตัว (ตรวจซ้ำที่เซิร์ฟเวอร์) |
| 6 | ทุก action เชื่อ `_userRole` ที่หน้าเว็บส่งมา = อ้างเป็นแอดมินได้ | ⚠️ ปิดแล้วเฉพาะคำสั่งอันตราย (ลบ/อัปโหลด/คลังข้อมูล) ด้วย session token — คำสั่งบันทึกทีมที่เหลือยังเชื่อ `_userGuild` อยู่ |
| 7 | `deleteImages` ลบไฟล์ใดก็ได้ใน Google Drive | ✅ แก้แล้ว — ลบได้เฉพาะไฟล์ที่อยู่ในโฟลเดอร์ของแอป |
| 8 | รหัสผ่านในชีต `Users` เก็บเป็นข้อความธรรมดา | ⚠️ ยังไม่แก้ |
