...
// =========================================================
// ⚙️ ค่าตั้ง — อ่านจาก Script Properties ก่อน ถ้าไม่มีค่อยใช้ค่าในโค้ดข้างล่าง
//
// ⚠️ ไฟล์นี้มี ID จริงอยู่ — กันหลุดด้วย .gitignore (apps-script/) ห้ามเอาออก
//    ถ้าจะให้ไฟล์นี้ขึ้น git สาธารณะ — ล้างค่าใน var ด้านล่างให้เป็น '' แล้วใช้ Script Properties อย่างเดียว
//
// เช็คว่าครบไหม: รัน configStatus()
// ย้ายค่าในโค้ดไป Properties: รัน setupConfig()
// =========================================================
var CONFIG_KEYS = ['SHEET_ID', 'IMAGE_FOLDER_ID', 'VIDEO_FOLDER_ID', 'SKILL_FOLDER_ID', 'EQUIP_FOLDER_ID', 'HERO_IMAGE_FOLDER_ID', 'RING_FOLDER_ID'];

var SHEET_ID = '1jwsNSHRNZ62r1hsO6UXJ-NMHASGQJDqvhqQJL4zhiew';
var IMAGE_FOLDER_ID = '1aoMOA52d1Ojn4Cr97PKMnSCK2cg1IB8D';
var VIDEO_FOLDER_ID = '1jTnrjTdNLtw9E58TB_sQ5ShELr-6uPrY';
var SKILL_FOLDER_ID = '12S_ycyylGP6T3oQVLtwkoRJwUNVmM-33';
var EQUIP_FOLDER_ID = '14Ci7SczLyh1rg071_jMIP03j0DjSu5Ki';
var HERO_IMAGE_FOLDER_ID = '1dg4tEQHjhnRd41hqxI2S8oLh-Fv0uR2M';
var RING_FOLDER_ID = '1OdiSJQz8Our2k2WSGEburQTHB_TNrLdW';

/** 📌 ค่าสำรอง = ค่าที่ประกาศไว้ข้างบน
 *  จำไว้ก่อน initConfig_() จะเขียนทับ — จะได้ใช้ต่อเมื่อ Script Properties ไม่มีค่า */
var CONFIG_FALLBACK = {
  SHEET_ID: SHEET_ID,
  IMAGE_FOLDER_ID: IMAGE_FOLDER_ID,
  VIDEO_FOLDER_ID: VIDEO_FOLDER_ID,
  SKILL_FOLDER_ID: SKILL_FOLDER_ID,
  EQUIP_FOLDER_ID: EQUIP_FOLDER_ID,
  HERO_IMAGE_FOLDER_ID: HERO_IMAGE_FOLDER_ID,
  RING_FOLDER_ID: RING_FOLDER_ID
};

/** โหลดค่าตั้งจาก Script Properties — เรียกทุกครั้งที่ doGet/doPost ทำงาน */
function initConfig_() {
  var props = PropertiesService.getScriptProperties().getProperties();
  function cfg(k) { return props[k] || CONFIG_FALLBACK[k] || ''; }
  SHEET_ID = cfg('SHEET_ID');
  IMAGE_FOLDER_ID = cfg('IMAGE_FOLDER_ID');
  VIDEO_FOLDER_ID = cfg('VIDEO_FOLDER_ID');
  SKILL_FOLDER_ID = cfg('SKILL_FOLDER_ID');
  EQUIP_FOLDER_ID = cfg('EQUIP_FOLDER_ID');
  HERO_IMAGE_FOLDER_ID = cfg('HERO_IMAGE_FOLDER_ID');
  RING_FOLDER_ID = cfg('RING_FOLDER_ID');
  return !!SHEET_ID;
}

/** 🔒 โฟลเดอร์อัปโหลด — client ส่งมาแค่ "ชื่อกลุ่ม" ไม่ใช่ ID ดิบ
 *  กันคนสั่งอัปไฟล์เข้าโฟลเดอร์อื่นใน Drive ของเรา */
function folderIdByKey_(key) {
  var map = {
    image: IMAGE_FOLDER_ID,
    video: VIDEO_FOLDER_ID,
    skill: SKILL_FOLDER_ID,
    equip: EQUIP_FOLDER_ID,
    hero: HERO_IMAGE_FOLDER_ID,
    ring: RING_FOLDER_ID
  };
  return map[String(key || '').toLowerCase()] || '';
}

/** 🛠️ ตั้งค่าครั้งแรก — กรอกค่าของตัวเองแล้ว Run หนึ่งครั้ง แล้วลบค่าออกก่อน commit */
function setupConfig() {
  var values = {
    SHEET_ID: CONFIG_FALLBACK.SHEET_ID,
    IMAGE_FOLDER_ID: CONFIG_FALLBACK.IMAGE_FOLDER_ID,
    VIDEO_FOLDER_ID: CONFIG_FALLBACK.VIDEO_FOLDER_ID,
    SKILL_FOLDER_ID: CONFIG_FALLBACK.SKILL_FOLDER_ID,
    EQUIP_FOLDER_ID: CONFIG_FALLBACK.EQUIP_FOLDER_ID,
    HERO_IMAGE_FOLDER_ID: CONFIG_FALLBACK.HERO_IMAGE_FOLDER_ID,
    RING_FOLDER_ID: CONFIG_FALLBACK.RING_FOLDER_ID,
    ADMIN_SECRET: ''   // 🔐 กรอกเองตรงนี้ → Run → ลบออก (ห้ามเก็บไว้ในโค้ด)
  };
  ...
  return saved.length ? ('บันทึกแล้ว: ' + saved.join(', ')) : 'ยังไม่ได้กรอกค่าอะไรเลย';
}

/** เช็คว่าค่าตั้งครบไหม (ไม่แสดงค่าจริง) */
function configStatus() {
  var props = PropertiesService.getScriptProperties().getProperties();
  return CONFIG_KEYS.concat(['ADMIN_SECRET', 'TOKEN_SECRET', 'SESSION_EPOCH']).map(function (k) {
    return (props[k] ? '✅ ' : '❌ ') + k;
  }).join(', ');
}
...
//    • ห้ามเชื่อ request._userRole / _userGuild ที่ client ส่งมา — ปลอมได้
//    • สิทธิ์อยู่ใน payload ที่ server เซ็นไว้ — แก้นิดเดียวลายเซ็นก็ไม่ตรง
//    • กุญแจสร้างอัตโนมัติครั้งแรก เก็บใน Script Properties — ไม่มีในโค้ด (ขึ้น git ได้)
//    • เตะทุกคนออก: รัน revokeAllSessions() ใน Apps Script editor
// =========================================================
var TOKEN_TTL_MS = 30 * 24 * 60 * 60 * 1000;   // 30 วัน

/** กุญแจเซ็น — สร้างอัตโนมัติครั้งแรกที่เรียกใช้ */
...
/** เลขรุ่น session — บวกทีหนึ่ง = token เก่าใช้ไม่ได้ทั้งหมด */
function sessionEpoch_() {
  var props = PropertiesService.getScriptProperties();
  var e = props.getProperty('SESSION_EPOCH');
  if (!e) { e = '1'; props.setProperty('SESSION_EPOCH', e); }
  return String(e);
}

/** เตะทุกคนออกจากระบบ — รันมือจาก Apps Script editor */
function revokeAllSessions() {
  var props = PropertiesService.getScriptProperties();
  var next = (parseInt(props.getProperty('SESSION_EPOCH'), 10) || 1) + 1;
  props.setProperty('SESSION_EPOCH', String(next));
  return 'session ทั้งหมดถูกยกเลิก — epoch ใหม่ = ' + next;
}

function signPayload_(json) {
  return Utilities.base64EncodeWebSafe(Utilities.computeHmacSha256Signature(json, tokenSecret_()));
}
...
function issueToken_(user) {
  var payload = JSON.stringify({
    ...
    g: String(user.guild_name || 'all'),
    e: sessionEpoch_(),
    x: Date.now() + TOKEN_TTL_MS
  });
  return Utilities.base64EncodeWebSafe(Utilities.newBlob(payload).getBytes()) + '.' + signPayload_(payload);
}

/** อ่านสิทธิ์จริงจาก token — คืน null ถ้าลายเซ็นไม่ตรง/หมดอายุ/โดนยกเลิก */
function authOf_(request) {
  var t = request && request._token;
  if (!t) return null;
  try {
    var parts = String(t).split('.');
    if (parts.length !== 2) return null;
    var json = Utilities.newBlob(Utilities.base64DecodeWebSafe(parts[0])).getDataAsString();
    if (signPayload_(json) !== parts[1]) return null;          // ลายเซ็นไม่ตรง = ของปลอม
    var pl = JSON.parse(json);
    if (!pl || !pl.x || Date.now() > pl.x) return null;        // หมดอายุ
    if (String(pl.e) !== sessionEpoch_()) return null;         // โดน revokeAllSessions
    return { id: pl.i, role: pl.r, guild: pl.g };
  } catch (e) { return null; }
}

function isAdminReq_(request) {
  var a = authOf_(request);
  return !!(a && a.role === 'admin');
}

function denyOut_(msg) {
  return out({ status: 'error', code: 'forbidden', message: msg || 'ไม่มีสิทธิ์ — กรุณาเข้าสู่ระบบใหม่' });
}

/** ลบ accessCode ออกจากก้อนข้อมูลก่อนส่งให้คนที่ไม่ใช่แอดมิน */
function stripAccessCodes_(data) {
  if (!data) return data;
  var clean = function (m) { if (m && m.accessCode !== undefined) m.accessCode = ''; };
  if (Array.isArray(data.guildMembers)) data.guildMembers.forEach(clean);
  if (Array.isArray(data.guilds)) data.guilds.forEach(function (g) {
    if (g && Array.isArray(g.members)) g.members.forEach(clean);
  });
  return data;
}
...
function doGet(e) {
  ...
  // 🔗 บอก URL ของ Web App ตัวเองให้หน้าเว็บรู้ — จะได้ไม่ต้องฝัง URL ไว้ในโค้ด (ไม่ขึ้น git)
  //    เปิดผ่าน Apps Script = ได้ค่านี้อัตโนมัติ · เปิดไฟล์ตรงๆ = ค่านี้ใช้ไม่ได้ หน้าเว็บจะถามเอง
  var selfUrl = '';
  try { selfUrl = ScriptApp.getService().getUrl() || ''; } catch (urlErr) {}
  template.webAppUrl = selfUrl;

  // 🔗 Apps Script เสิร์ฟหน้านี้ใน iframe — query ที่ต่อท้าย /exec (?k= ?g= ?mode=) ไม่ติดมาด้วย
  //    doGet จึงส่งมาให้เป็น base64 — เปิดไฟล์ตรงๆ ค่านี้เป็น template ดิบ appUrlParams() จะข้ามให้เอง
  var urlQuery = '';
  try {
    var qp = (e && e.parameter) ? e.parameter : {};
    var parts = [];
    Object.keys(qp).forEach(function (k) { parts.push(encodeURIComponent(k) + '=' + encodeURIComponent(qp[k])); });
    urlQuery = Utilities.base64Encode(parts.join('&'), Utilities.Charset.UTF_8);
  } catch (qErr) {}
  template.urlQuery = urlQuery;
...
// 📖 คำสั่งที่ "อ่านอย่างเดียว" — ไม่ต้องเข้าคิวรอล็อก
//    ของเดิมล็อกทุกคำขอ = คนเปิดแกลเลอรี/ลิงก์แชร์พร้อมกันต้องรอกันเป็นแถว
//    คนแรกอ่านข้อมูลก้อนใหญ่ 15-20 วิ คนถัดไปรอจนหมดเวลา → "การเชื่อมต่อล้มเหลว"
var READ_ONLY_ACTIONS = [
  'getAllData', 'getUpdates', 'getVersions', 'getPublicConfig',
  'getAllSharedTeams', 'getSharedTeam', 'getCounterSnapshot',
  'getUserTier', 'getWarTeams', 'getMemberInventory',
  'getDeleted3v3Teams', 'getDriveFolderLink',
  'getRosterBootstrap',
  'verifyMemberCode', 'verifyAdmin'
];

// =========================================================
// 🔒 คำสั่งที่เปิดสาธารณะ — นอกจากนี้ต้อง login ทั้งหมด
//    แก้เฉพาะที่นี่ = หน้าที่คนนอกต้องใช้จริง (ลิงก์แชร์ / แกลเลอรี / ใบสมัคร / พอร์ทัลลูกกิล)
//    พอร์ทัลลูกกิลไม่มี user/pass — ยืนยันตัวด้วยรหัสสมาชิกแทน
// =========================================================
var PUBLIC_ACTIONS = [
  'login', 'verifyAdmin',
  'getPublicConfig', 'submitApplication',
  'getSharedTeam', 'getAllSharedTeams', 'getCounterSnapshot',
  'getRosterBootstrap', 'verifyMemberCode', 'getMemberInventory', 'saveMemberInventory'
];

function doPost(e) {
  // อ่าน action ก่อน เพื่อตัดสินใจว่าต้องล็อกไหม
  var request, action;
  try {
    request = JSON.parse(e.postData.contents);
    action = request.action;
  } catch (parseErr) {
    return out({ status: 'error', message: 'รูปแบบคำขอไม่ถูกต้อง' });
  }

  var lock = null;
  if (READ_ONLY_ACTIONS.indexOf(action) < 0) {
    lock = LockService.getScriptLock();
    // รอสูงสุด 30 วินาที (เฉพาะคำสั่งที่เขียนข้อมูล)
    try {
      lock.waitLock(30000);
    } catch (lockErr) {
      return out({ status: 'error', code: 'busy', message: 'มีคนกำลังบันทึกข้อมูลอยู่ — รอสักครู่แล้วลองใหม่' });
    }
  }

  try {
    if (!initConfig_()) {
      return out({ status: 'error', code: 'no_config', message: 'ยังไม่ได้ตั้งค่า SHEET_ID ใน Script Properties — เปิด Apps Script แล้วรัน setupConfig()' });
    }
    ...
    // =========================================================
    // 🔐 ด่านตรวจสิทธิ์ — action อันตรายต้องมี token จริงจาก login
    //    (ไม่ใช่ _userRole ที่ client ส่งมา — อันนั้นปลอมได้)
    // =========================================================
    var ADMIN_ONLY_ACTIONS = [
      'purge3v3Team', 'restore3v3Team', 'getDeleted3v3Teams',
      'deleteImages', 'saveCounterSnapshot',
      'saveOneItem', 'deleteOneItem', 'saveManySkillLib'
    ];
    // คำสั่งที่ต้อง login — ทั้งดึงข้อมูลและเขียนข้อมูล
    //    สิทธิ์จริงอ่านจาก token — ไม่เชื่อค่าที่ client ส่งมา
    var LOGIN_ONLY_ACTIONS = [
      // 📥 ดึงข้อมูลระบบ — คนนอกยิงไม่ได้อีกแล้ว
      'getAllData', 'getUpdates', 'getVersions', 'getUserTier',
      'getWarTeams', 'saveWarTeam', 'deleteWarTeam',
      'getDriveFolderLink',
      // 📤 อัปโหลดไฟล์
      'uploadVideo', 'uploadSkillImage', 'uploadEquipImage', 'uploadHeroImage', 'uploadRingImage',
      // ✏️ เขียนทีม/ศัตรู
      'saveOne3v3Team', 'deleteOne3v3Team', 'saveMany3v3Teams',
      'saveOneEnemyPattern', 'deleteOneEnemyPattern', 'saveManyEnemyPatterns',
      'shareTeam'
    ];
    // คำสั่งที่เขียนทับทั้งชีต — แอดมินเท่านั้น
    var ADMIN_BULK_ACTIONS = ['save3v3Teams', 'saveEnemyPatterns'];
    if (ADMIN_BULK_ACTIONS.indexOf(action) >= 0 && !isAdminReq_(request)) {
      return denyOut_('คำสั่งนี้ใช้ได้เฉพาะแอดมิน: ' + action);
    }
    // 🔒 สิทธิ์จริงจาก token — ไม่แตะค่า _userRole/_userGuild ที่ client ส่งมาอีกเลย
    var AUTH = authOf_(request);
    request._userRole = AUTH ? AUTH.role : 'guest';
    request._userGuild = AUTH ? AUTH.guild : 'all';

    if (ADMIN_ONLY_ACTIONS.indexOf(action) >= 0 && !isAdminReq_(request)) {
      return denyOut_('คำสั่งนี้ใช้ได้เฉพาะแอดมิน: ' + action);
    }
    if (LOGIN_ONLY_ACTIONS.indexOf(action) >= 0 && !AUTH) {
      return denyOut_('ต้องเข้าสู่ระบบก่อน: ' + action);
    }
    // 🔒 กำแพงสุดท้าย — action ใหม่ที่ไม่ได้อยู่ใน PUBLIC_ACTIONS ต้อง login เสมอ
    //    (กันลืมใส่ใน LOGIN_ONLY_ACTIONS แล้วหลุดเปิดสาธารณะโดยไม่ตั้งใจ)
    if (PUBLIC_ACTIONS.indexOf(action) < 0 && !AUTH) {
      return denyOut_('ต้องเข้าสู่ระบบก่อน: ' + action);
    }
    ...
    if (action === 'verifyAdmin') {
     ...
     // 🔐 รหัสอยู่ใน Script Properties ชื่อ ADMIN_SECRET — ไม่มีในโค้ด
     var myRealSecret = PropertiesService.getScriptProperties().getProperty('ADMIN_SECRET') || '';
     if (!myRealSecret) return out({ status: 'error', message: 'ยังไม่ได้ตั้งค่า ADMIN_SECRET' });
    ...
    // 🔑 พอร์ทัลลูกกิล: ตรวจรหัสสมาชิกฝั่ง server แล้วบอกว่าเป็นใคร
    //    คืนแค่ข้อมูลของคนนั้นคนเดียว — ไม่คายรายชื่อ/รหัสของคนอื่น
    // =========================================================
    // 🏰 พอร์ทัลลูกกิล (?mode=roster) — ข้อมูลเท่าที่จำเป็นตอนเปิดหน้า
    //    ของเดิมใช้ getAllData = ได้ข้อมูลทั้งระบบ (สมาชิกทุกคน รหัส ทีม ประวัติ)
    //    อันนี้ส่งแค่: ข้อมูลกลาง (ฮีโร่/สัตว์/แหวน/อุปกรณ์) + รายชื่อกิลด์ (id/name)
    //    ไม่มีสมาชิก ไม่มีรหัส ไม่มีทีม — คนนอกยิงมาก็ได้แค่ของสาธารณะ
    // =========================================================
    if (action === 'getRosterBootstrap') {
      var rbServerVersions = computeServerVersions_(ss);
      var rbGuilds = loadFromSheet(ss, 'Guilds', []).map(function (g) {
        return { id: String(g.id || ''), name: g.name || '', type: g.type || 'sub', members: [] };
      });
      return out({
        status: 'success',
        guilds: rbGuilds,
        masterData: getPublicMasterData_(ss, request.versions || {}, rbServerVersions),
        versions: rbServerVersions,
        timestamp: Date.now()
      });
    }

    if (action === 'verifyMemberCode') {
       var vCode = String(request.accessCode || '').trim();
       if (vCode.length < 6) return denyOut_('รหัสสมาชิกต้องยาวอย่างน้อย 6 ตัว');
       var vRows = loadFromSheet(ss, 'GuildMembers', []);
       var vHit = null;
       for (var vi = 0; vi < vRows.length; vi++) {
         if (memberCodesMatchServer(vRows[vi].accessCode, vCode)) { vHit = vRows[vi]; break; }
       }
       if (!vHit) return denyOut_('รหัสไม่ถูกต้อง');
       var vGuilds = loadFromSheet(ss, 'Guilds', []);
       var vGuildName = '';
       for (var vg = 0; vg < vGuilds.length; vg++) {
         if (String(vGuilds[vg].id) === String(vHit.guild_id)) { vGuildName = vGuilds[vg].name || ''; break; }
       }
       return out({
         status: 'success',
         guild_id: String(vHit.guild_id || 'G_MAIN'),
         guild_name: vGuildName,
         member_id: String(vHit.id || ''),
         name: vHit.name || ''
       });
    }

    // 📁 ลิงก์โฟลเดอร์คลังรูปใน Drive — แอดมินเท่านั้น (ไม่ส่ง ID ให้คนทั่วไป)
    if (action === 'getDriveFolderLink') {
       if (!isAdminReq_(request)) return denyOut_('เปิดคลังรูปได้เฉพาะแอดมิน');
       var fid = folderIdByKey_(request.folderKey);
       if (!fid) return out({ status: 'error', message: 'ยังไม่ได้ตั้งค่าโฟลเดอร์นี้ใน Script Properties' });
       return out({ status: 'success', url: 'https://drive.google.com/drive/folders/' + fid });
    }
    ...
    if (action === 'login') {
      ...
      if (user) {
        user.token = issueToken_(user);   // 🔐 ออก token ให้ client แนบกลับมาทุกครั้ง
    ...
    // =========================================================
        ...
        if (foundTeam) {
            // ⚡ ส่งข้อมูลกลางเฉพาะหมวดที่ client ยังไม่มี/เก่ากว่า (ครั้งแรกได้ครบ ครั้งต่อไปมักว่าง)
            var stVersions = computeServerVersions_(ss);
            var masterData = getPublicMasterData_(ss, request.versions, stVersions);
            console.log('🔗 getSharedTeam — categories refreshed:', Object.keys(masterData));
            return out({ status: 'success', team: foundTeam, masterData: masterData, versions: stVersions });
        } else {
    ...
    // 🟢 ส่งข้อมูลทั้งหมดที่ Share=true ไม่กรอง Guild (ให้ Frontend กรองเอง)
        ...
        // ⚡ เทียบเวอร์ชันก่อนแตะชีต — ทีมไม่เปลี่ยน = ไม่ต้องอ่าน Teams3v3 ทั้งแผ่น
        //    (client เก็บทีมไว้ในเครื่องแล้ว ส่ง teams: null กลับไปพอ)
        var clientVersions = request.versions || {};
        var serverVersions = computeServerVersions_(ss);
        var teamsUpToDate = (clientVersions.teams_3v3 || 0) >= serverVersions.teams_3v3;

        var s = teamsUpToDate ? null : ss.getSheetByName('Teams3v3');
        var teamList = teamsUpToDate ? null : [];
        ...
        console.log('gallery: teams=' + (teamList === null ? 'unchanged' : teamList.length)
                    + ' | master refreshed: ' + (Object.keys(masterData).join(',') || 'none'));
    ...
    // =========================================================
      ...
      // 🔐 รหัสสมาชิก (accessCode) ส่งให้เฉพาะแอดมิน
      if (!isAdminReq_(request)) data = stripAccessCodes_(data);
      return out(data);
    }
    ...
    // ⚡ INCREMENTAL: ส่งเฉพาะ category ที่เปลี่ยน (เทียบกับเวอร์ชันที่ client มี)
      ...
      var changed = {};
      var vIsAdmin = isAdminReq_(request);
      Object.keys(serverV).forEach(function (cat) {
        if ((clientV[cat] || 0) < serverV[cat]) {
          var d = loadCategoryData_(ss, cat);
          if (d === null) return;
          // 🔐 ตัด accessCode ออกจาก guilds ถ้าไม่ใช่แอดมิน
          if (!vIsAdmin && cat === 'guilds' && Array.isArray(d)) {
            d.forEach(function (g) {
              if (g && Array.isArray(g.members)) g.members.forEach(function (m) { if (m && m.accessCode !== undefined) m.accessCode = ''; });
            });
          }
          changed[cat] = d;
        }
    ...
    // =========================================================
      ...
      return out({ status: 'success', surveyConfig: conf });
       ...
       // 🔒 ไม่รับ folderId ดิบจาก client — กันอัปไฟล์เข้าโฟลเดอร์อื่นใน Drive
       var eFolder = folderIdByKey_(request.folderKey) || EQUIP_FOLDER_ID;
       ...
       var hFolder = folderIdByKey_(request.folderKey) || HERO_IMAGE_FOLDER_ID;
    ...
    // =========================================================
      ...
      if (!gId) gId = 'G_MAIN';
      // 🔐 คนทั่วไปต้องมีรหัสสมาชิก — ส่งแค่ member_id มาดึงของคนอื่นไม่ได้
      if (!isAdminReq_(request)) {
        if (!aCode || aCode.length < 6) return denyOut_('ต้องใส่รหัสสมาชิก (อย่างน้อย 6 ตัว)');
        mId = '';   // ให้จับคู่ด้วยรหัสอย่างเดียว
      }
    ...
    // =========================================================
      ...
      // 🔐 คนทั่วไปต้องมีรหัสสมาชิกที่ตรงจริง ถึงเขียนข้อมูลได้
      //    และเขียนได้แค่แถวของรหัสนั้น — ส่ง member_id ของคนอื่นมาไม่มีผล
      //    แอดมิน (มี token) ข้ามการตรวจนี้ได้ — หน้าจัดทีมลูกกิลต้องใช้
      if (!isAdminReq_(request)) {
        if (!accessCode || accessCode.length < 6) {
          return denyOut_('ต้องใส่รหัสสมาชิก (อย่างน้อย 6 ตัว)');
        }
        var owned = loadAllMatchingMemberRowsFromSheet(ss, guildId, '', accessCode);
        if (!owned.length) return denyOut_('รหัสสมาชิกไม่ถูกต้อง');
        memberId = String(owned[0].id);   // บังคับเขียนทับได้เฉพาะแถวตัวเอง
      }
    ...
    // =========================================================
       ...
       // 🔒 คำสั่งบันทึกก้อนใหญ่ (เขียนได้ทุกชีต รวมชีต Users) — ต้อง login ก่อน
       //    ส่วนที่แตะชีต Users / Logs จำกัดเฉพาะแอดมิน
       if (!AUTH) return denyOut_('ต้องเข้าสู่ระบบก่อนบันทึกข้อมูล');
       if ((request.users || request.auditLogs) && !isAdminReq_(request)) {
         return denyOut_('แก้ไขผู้ใช้/ประวัติได้เฉพาะแอดมิน');
       }
...
// 📦 หมวดข้อมูลกลางที่หน้าสาธารณะ (แชร์ลิงก์/แกลเลอรี) ต้องใช้วาดการ์ด
var PUBLIC_MASTER_CATS = ['heroes', 'pets', 'rings', 'equip_sets', 'accessories', 'skillLib'];

/**
 * 📦 ข้อมูลกลางแบบ "ส่งเท่าที่เปลี่ยน"
 *    client ส่งเวอร์ชันที่ตัวเองมีมา (request.versions) → ส่งกลับเฉพาะหมวดที่ใหม่กว่า
 *    ครั้งแรกไม่มีเวอร์ชัน = ได้ครบทุกหมวด · ครั้งต่อไปมักได้ {} (ไม่ต้องโหลดซ้ำ)
 *    ไม่มี guilds / guildMembers / battleHistory / applications ติดไปด้วย
 */
function getPublicMasterData_(ss, clientVersions, serverVersions) {
  var cv = clientVersions || {};
  var sv = serverVersions || computeServerVersions_(ss);
  var outData = {};
  PUBLIC_MASTER_CATS.forEach(function (cat) {
    if ((cv[cat] || 0) < sv[cat]) outData[cat] = loadCategoryData_(ss, cat);
  });
  return outData;
}
...
// =========================================================
// 🔒 รหัสผ่านผู้ใช้ — เก็บเป็น hash ไม่ใช่ข้อความธรรมดา
//    รูปแบบที่เก็บในชีต: sha256$<salt>$<hash>
//    รหัสเก่าที่ยังเป็นข้อความธรรมดา ยัง login ได้ และจะถูกแปลงเป็น hash ให้อัตโนมัติ
// =========================================================
...
function makePasswordHash_(plain) {
  ...
  return 'sha256$' + salt + '$' + hashPassword_(plain, salt);
}
...
function passwordMatches_(stored, plain) {
  var v = String(stored || '');
  if (!isHashedPassword_(v)) return v === String(plain);   // ของเก่า: เทียบตรงๆ
  var parts = v.split('$');
  if (parts.length !== 3) return false;
  return parts[2] === hashPassword_(plain, parts[1]);
}

/** แปลงรหัสธรรมดาในชีตให้เป็น hash (เรียกตอน login ผ่านด้วยรหัสเก่า) */
function upgradePasswordToHash_(ss, userId, plain) {
  try {
    ...
    for (var i = 0; i < rows.length; i++) {
      if (String(rows[i][0]) === String(userId)) {
        sheet.getRange(i + 1, 3).setValue(makePasswordHash_(plain));
        return;
      }
    }
  } catch (e) { Logger.log('upgradePasswordToHash_ failed: ' + e); }
}

function verifyUser(ss, u, p) {
  // ⚠️ ไม่ log ชื่อผู้ใช้/role/รหัส — log ของ Apps Script คนอื่นอาจเห็น
  if (!String(u || '').trim() || !String(p || '').trim()) return null;   // 🔒 กันรหัสว่าง
  var users = loadUsersFromSheet(ss);
  for (var i = 0; i < users.length; i++) {
    if (String(users[i].id) !== String(u)) continue;
    if (!passwordMatches_(users[i].pass, p)) return null;
    var found = users[i];
    // รหัสเก่ายังเป็นข้อความธรรมดา → แปลงเป็น hash ให้เลย (ครั้งเดียว)
    if (!isHashedPassword_(found.pass)) upgradePasswordToHash_(ss, found.id, p);
    delete found.pass;
    return found;
  }
  return null;
...
function saveUsersToSheet(ss, users) {
  ...
  // 🔒 เก็บเป็น hash เสมอ — ถ้าเป็น hash อยู่แล้วก็ใช้ค่าเดิม (ไม่ hash ซ้อน)
  var r = users.map(function(u) {
    var pass = String(u.pass == null ? '' : u.pass);
    var stored = isHashedPassword_(pass) ? pass : (pass ? makePasswordHash_(pass) : '');
    return [u.id, u.name, stored, u.role, JSON.stringify(u.permissions||[]), u.guild_name || 'all'];
  });
...
/** 🔒 ไฟล์นี้อยู่ในโฟลเดอร์ของแอปจริงไหม — กันสั่งลบไฟล์อื่นใน Drive */
function isFileInAppFolders_(file) {
  var allowed = [IMAGE_FOLDER_ID, VIDEO_FOLDER_ID, SKILL_FOLDER_ID, EQUIP_FOLDER_ID, HERO_IMAGE_FOLDER_ID, RING_FOLDER_ID];
  try {
    var parents = file.getParents();
    while (parents.hasNext()) {
      if (allowed.indexOf(parents.next().getId()) >= 0) return true;
    }
  } catch (e) {}
  return false;
}
...
function deleteImagesFromDrive(urls) {
  ...
  urls.forEach(function(url) {
    try {
      ...
      // 🔒 ลบได้เฉพาะไฟล์ที่แอปอัปเอง — ไฟล์ส่วนตัวใน Drive ห้ามแตะ
      if (!isFileInAppFolders_(file)) { Logger.log('blocked delete outside app folders: ' + match[0]); return; }
